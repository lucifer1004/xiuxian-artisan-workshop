...
def _ensure_agent_binary(agent_bin: Path) -> None:
    ...
    subprocess.run(
        ["cargo", "build", "-p", "xiuxian-daochang", "--bin", "xiuxian-daochang"],
        cwd=_ROOT,
        check=True,
        env=env,
    )
    if not agent_bin.exists():
        raise RuntimeError(f"xiuxian-daochang binary not found after build: {agent_bin}")
...
async def _run_role_benchmark(
    ...
    log_path = temp_root / f"xiuxian-daochang-{role.name}.log"
...
async def _main() -> int:
    parser = argparse.ArgumentParser(
        description="Role-oriented embedding performance smoke benchmark for xiuxian-daochang."
    )
    ...
    parser.add_argument(
        "--agent-bin",
        type=Path,
        default=_ROOT / "target" / "debug" / "xiuxian-daochang",
    )
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="xiuxian-daochang-role-perf-"))
    started = time.time()
    try:
        results = []
