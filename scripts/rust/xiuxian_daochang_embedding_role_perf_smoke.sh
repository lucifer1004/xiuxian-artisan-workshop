...
if [[ -n ${report_json} ]]; then
  env_args+=("XIUXIAN_DAOCHANG_EMBED_ROLE_PERF_REPORT=${report_json}")
fi

env "${env_args[@]}" "${cargo_bin}" test -p xiuxian-daochang --test embedding_role_perf_smoke -- --ignored --nocapture
