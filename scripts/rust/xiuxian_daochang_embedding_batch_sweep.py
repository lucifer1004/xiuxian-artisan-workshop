...
This script starts an isolated xiuxian-daochang gateway for each `max_num_seqs` value,
...
DEFAULT_AGENT_BIN = ROOT / "target" / "debug" / "xiuxian-daochang"
...
def start_gateway(
    ...
    if not agent_bin.exists():
        raise FileNotFoundError(f"xiuxian-daochang binary not found: {agent_bin}")
...
async def run(args: argparse.Namespace) -> int:
    ...
    report_json = report_dir / f"xiuxian-daochang-embed-batch-sweep-{now}.json"
    report_md = report_dir / f"xiuxian-daochang-embed-batch-sweep-{now}.md"
    latest_json = report_dir / "xiuxian-daochang-embed-batch-sweep-latest.json"
    latest_md = report_dir / "xiuxian-daochang-embed-batch-sweep-latest.md"
    ...
    for index, max_num_seqs in enumerate(args.max_num_seqs):
        ...
        with tempfile.TemporaryDirectory(prefix=f"embed-sweep-{max_num_seqs}-") as tmp:
            ...
            log_path = (
                ROOT / ".run" / "logs" / f"xiuxian-daochang-embed-batch-sweep-{max_num_seqs}.log"
            )
    ...
    report = {
        "schema": "xiuxian_daochang.embed_batch_sweep.v1",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep xiuxian-daochang mistral-sdk /embed/batch performance"
    )
