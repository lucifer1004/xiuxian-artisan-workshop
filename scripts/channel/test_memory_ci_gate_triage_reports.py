...
def test_write_gate_failure_triage_report_writes_expected_sections(tmp_path) -> None:
    ...
    cfg.runtime_log_file.write_text(
        '2026-02-22T00:00:00Z WARN event="tool_runtime.pool.call.waiting"\n',
        encoding="utf-8",
    )
    report = write_gate_failure_triage_report(
        cfg,
        error=RuntimeError("tool waiting warning budget exceeded"),
        category="tool_waiting_budget",
        summary="tool waiting warning budget exceeded",
        repro_commands=["echo triage"],
    )
    content = report.read_text(encoding="utf-8")
    assert "Omni Agent Memory CI Failure Triage" in content
    assert "category: `tool_waiting_budget`" in content
...
def test_write_gate_failure_triage_json_report_writes_expected_payload(tmp_path) -> None:
    ...
    assert payload["category"] == "tool_waiting_budget"
    assert payload["summary"] == "tool waiting warning budget exceeded"
