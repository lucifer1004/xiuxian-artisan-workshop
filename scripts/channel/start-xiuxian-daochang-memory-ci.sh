...
usage() {
  cat <<'EOF'
Usage: start-xiuxian-daochang-memory-ci.sh --profile <quick|nightly> [options] [-- <gate-args...>]
...
All unknown options after "--" are passed to:
  test_xiuxian_daochang_memory_ci_gate.py --profile <profile> ...
EOF
}
...
if [[ -x "${PROJECT_ROOT}/target/debug/xiuxian-daochang" ]]; then
  AGENT_BIN="${PROJECT_ROOT}/target/debug/xiuxian-daochang"
fi

RUN_STAMP="$("$PYTHON_BIN" scripts/channel/epoch_millis.py)"
LOG_FILE=""
LATEST_FAILURE_JSON="${REPORTS_DIR}/xiuxian-daochang-memory-ci-latest-failure.json"
LATEST_FAILURE_MD="${REPORTS_DIR}/xiuxian-daochang-memory-ci-latest-failure.md"
LATEST_RUN_JSON="${REPORTS_DIR}/xiuxian-daochang-memory-ci-latest-run.json"
PID_FILE=""
...
if [[ -z $LOG_FILE ]]; then
  LOG_FILE="${LOGS_DIR}/xiuxian-daochang-memory-ci-${PROFILE}-${RUN_STAMP}.log"
fi
if [[ -z $PID_FILE ]]; then
  PID_FILE="${STATE_DIR}/xiuxian-daochang-memory-ci-${PROFILE}.pid"
fi
...
if [[ $AUTO_AGENT_BIN == "true" && -z $AGENT_BIN && -x "${PROJECT_ROOT}/target/debug/xiuxian-daochang" ]]; then
  AGENT_BIN="${PROJECT_ROOT}/target/debug/xiuxian-daochang"
fi
...
GATE_CMD=(
  "$PYTHON_BIN"
  "${SCRIPT_DIR}/test_xiuxian_daochang_memory_ci_gate.py"
  "--profile"
  "$PROFILE"
)
if [[ -n $AGENT_BIN ]]; then
...
RUNNER_SCRIPT="${STATE_DIR}/xiuxian-daochang-memory-ci-${PROFILE}-runner-${RUN_STAMP}.sh"
