...
LAUNCHER_SCRIPT = PROJECT_ROOT / "scripts" / "channel" / "start-xiuxian-daochang-memory-ci-quick.sh"
...
def test_launcher_foreground_help_updates_latest_run(tmp_path: Path) -> None:
    ...
    assert "Quick memory CI gate completed successfully." in result.stdout
...
def test_launcher_foreground_invalid_gate_arg_writes_fallback_failure_payload(
    ...
    assert "Quick memory CI gate failed with exit code" in result.stdout
