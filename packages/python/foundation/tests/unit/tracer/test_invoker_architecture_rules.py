...
TRACER_DIR = Path(__file__).resolve().parents[3] / "src" / "xiuxian_tracer"
...
def test_node_factory_does_not_depend_on_concrete_invokers() -> None:
    ...
    forbidden = {"tool_invoker", "retrieval_invoker", "composite_invoker", "invoker_stack"}
...
def test_tool_invoker_depends_only_on_node_factory_contract() -> None:
    """tool_invoker should use ToolInvoker protocol and stay isolated."""
    imported = _imported_modules(_parse("tool_invoker.py"))
    ...
    for module in imported:
        assert not any(name in module for name in forbidden), (
            "tool_invoker.py must not import other invoker stack or pipeline modules"
        )


def test_retrieval_invoker_depends_only_on_contract_and_retrieval_backend() -> None:
    """retrieval_invoker should not couple to tool/composite/stack/pipeline."""
    imported = _imported_modules(_parse("retrieval_invoker.py"))
    assert any("node_factory" in module for module in imported)
    assert any("xiuxian_rag.retrieval" in module for module in imported)
    forbidden = {"tool_invoker", "composite_invoker", "invoker_stack", "pipeline"}
    for module in imported:
        assert not any(name in module for name in forbidden), (
            "retrieval_invoker.py must not import tool/composite/stack/pipeline modules"
        )
...
def test_composite_invoker_depends_only_on_tool_contract_layer() -> None:
    ...
    assert any("node_factory" in module for module in imported)
    forbidden = {"tool_invoker", "retrieval_invoker", "invoker_stack", "pipeline"}
    for module in imported:
        assert not any(name in module for name in forbidden), (
...
def test_invoker_stack_is_only_composition_layer() -> None:
    ...
    required = {"composite_invoker", "tool_invoker", "retrieval_invoker", "node_factory"}
