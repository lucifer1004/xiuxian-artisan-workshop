...
def test_node_factory_does_not_depend_on_concrete_invokers() -> None:
    ...
    forbidden = {"tool_invoker", "composite_invoker", "invoker_stack"}
...
def test_tool_invoker_depends_only_on_node_factory_contract() -> None:
    ...
    forbidden = {"composite_invoker", "invoker_stack", "pipeline"}
    for module in imported:
        assert not any(name in module for name in forbidden), (
...
def test_retrieval_invoker_module_is_removed() -> None:
    assert not (TRACER_DIR / "retrieval_invoker.py").exists()
...
def test_composite_invoker_depends_only_on_tool_contract_layer() -> None:
    ...
    assert any("node_factory" in module for module in imported)
    forbidden = {"tool_invoker", "invoker_stack", "pipeline"}
    for module in imported:
        assert not any(name in module for name in forbidden), (
...
def test_invoker_stack_is_only_composition_layer() -> None:
    ...
    required = {"composite_invoker", "tool_invoker", "node_factory"}
