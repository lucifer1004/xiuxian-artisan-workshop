"""
test_tool_invoker.py - Unit tests for ToolClientInvoker.
...
from xiuxian_tracer import PipelineConfig, ToolClientInvoker, create_workflow_from_pipeline
...
@pytest.mark.asyncio
async def test_tool_invoker_supports_positional_arguments_signature():
    invoker = ToolClientInvoker(_ClientPositional())
...
@pytest.mark.asyncio
async def test_tool_invoker_supports_keyword_signature():
    invoker = ToolClientInvoker(_ClientKeyword())
    result = await invoker.invoke(
        server="retriever",
        tool="search",
        payload={"query": "typed languages"},
...
@pytest.mark.asyncio
async def test_tool_invoker_normalizes_text_content_json():
    invoker = ToolClientInvoker(_ClientTextList())
    result = await invoker.invoke(
        server="retriever",
        tool="search",
        payload={"query": "typed languages"},
...
@pytest.mark.asyncio
async def test_tool_invoker_normalizes_canonical_dict_json():
    invoker = ToolClientInvoker(_ClientCanonicalDict())
    result = await invoker.invoke(
        server="retriever",
        tool="search",
        payload={"query": "typed languages"},
...
@pytest.mark.asyncio
async def test_tool_invoker_integrates_with_pipeline_graph():
    ...
    app = create_workflow_from_pipeline(
        config,
        state_schema=dict,
        tool_invoker=ToolClientInvoker(_ClientPositional()),
    )
