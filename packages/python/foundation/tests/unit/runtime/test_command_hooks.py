"""Unit tests for command execution lifecycle hooks."""

from __future__ import annotations

import pytest

from xiuxian_foundation.command_hooks import (
...
class TestCommandHooks:
    """Register and run before/after command execute callbacks."""
    ...
    def test_run_before_executes_registered_callback(self):
        """run_before_command_execute runs registered callbacks."""
        seen = []
        ...
        register_before_command_execute(cb1)
        register_before_command_execute(cb2)
        run_before_command_execute()
        assert 1 in seen and 2 in seen

    def test_run_after_executes_registered_callback(self):
        """run_after_command_execute runs registered callbacks."""
        seen = []

        def cb():
            seen.append("after")

        register_after_command_execute(cb)
        run_after_command_execute()
    ...
    def test_exception_in_callback_does_not_stop_others(self):
        ...
        register_before_command_execute(bad)
        register_before_command_execute(good)
        run_before_command_execute()
        assert "ok" in seen
