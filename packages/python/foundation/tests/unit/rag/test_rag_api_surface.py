"""API surface checks for `xiuxian_rag` namespace."""
...
def test_legacy_link_graph_client_module_removed() -> None:
    """Legacy `xiuxian_rag.link_graph_client` module should not be importable."""
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("xiuxian_rag.link_graph_client")


def test_importing_legacy_symbol_from_namespace_fails() -> None:
    """`from xiuxian_rag import LinkGraphClient` must fail after API cleanup."""
    with pytest.raises(ImportError):
        exec("from xiuxian_rag import LinkGraphClient", {})
