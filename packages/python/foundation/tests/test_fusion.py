...
def _setup_mock_knowledge_graph(
    ...
    mock_module.load_kg_from_valkey_cached = MagicMock(return_value=mock_kg)
...
class TestEnrichSkillGraphFromLinkGraph:
    ...
    def test_passthrough_when_no_scope_key(self, tmp_path: Path) -> None:
        original = {"git.commit": [("git.smart_commit", _MEDIUM_EDGE)]}
        result = enrich_skill_graph_from_link_graph(
            original, scope_key=str(tmp_path / "nonexistent.scope")
        )
        assert result == original

    def test_passthrough_empty_graph(self) -> None:
        result = enrich_skill_graph_from_link_graph({}, scope_key="/nonexistent")
        assert result == {}
...
class TestSkillCommandWiring:
    """Tests that skill commands properly use fusion bridges."""
    ...
    def test_recall_imports_fusion_boost(self, skills_dir: Path) -> None:
        mod = _import_skill_script(skills_dir, "knowledge", "recall.py")
        assert callable(mod._apply_fusion_recall_boost)

    @pytest.mark.asyncio
    async def test_recall_fusion_boost_graceful_fallback(self, skills_dir: Path) -> None:
        mod = _import_skill_script(skills_dir, "knowledge", "recall.py")

        results = [_make_result("a.md", _TOP_SCORE)]
        result = await mod._apply_fusion_recall_boost(results, "test")
...
class TestFusionWeights:
    ...
    def test_empty_query_returns_balanced(self):
        from omni.rag.fusion import FusionWeights, compute_fusion_weights
