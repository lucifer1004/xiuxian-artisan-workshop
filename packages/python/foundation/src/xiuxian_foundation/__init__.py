...
def __getattr__(name: str):
    ...
    if name in (
        ...
        if _workflow_state_module is None:
            _workflow_state_module = __import__("xiuxian_foundation.workflow_state", fromlist=[""])
    ...
    if name in ("SearchResult",):
        if _services_module is None:
            _services_module = __import__("xiuxian_foundation.services", fromlist=[""])
    ...
    if name == "bridge":
        if _bridge_module is None:
            _bridge_module = __import__("xiuxian_foundation.bridge", fromlist=[""])
        return _bridge_module

    # Lazy load api module (foundation decorators and utilities)
    if name == "api":
        if _api_module is None:
            _api_module = __import__("xiuxian_foundation.api", fromlist=[""])
        return _api_module
    ...
    if name in (
        ...
        if _rag_module is None:
            _rag_module = __import__("xiuxian_rag", fromlist=[""])
    ...
    if name in (
        ...
        if _embedding_client_module is None:
            _embedding_client_module = __import__("xiuxian_foundation.embedding_client", fromlist=[""])
