"""
tool_context.py - Unified execution context for long-running tool calls.

Single timeout interface: both service-side and CLI skill runners call
run_with_execution_timeout(coro). Config (`tool.timeout`, `tool.idle_timeout`)
is read in one place; no per-caller logic.

Usage:
  - Runner: call run_with_execution_timeout(coro). Do not read timeout config
    or call run_with_idle_timeout directly.
  - Skill (e.g. researcher): call run_with_heartbeat(coro) or heartbeat()
    during long work. Works identically across runners.
"""
...
# Context is set by the invoking runner before the tool starts. Tools read it
# and call heartbeat() during long work.
...
async def run_with_heartbeat(
    ...
    """Run a coroutine with periodic heartbeat so idle timeout does not kill long work.
...
async def run_with_idle_timeout(
    ...
    """Run a coroutine under the unified timeout and heartbeat framework.
...
def get_execution_timeout_config() -> tuple[float, float]:
    """Get (total_timeout_s, idle_timeout_s) from config."""
    try:
        total = get_setting("tool.timeout", 1800) or 1800
        idle = get_setting("tool.idle_timeout", 0) or 0
        total_s = float(total)
        idle_s = float(idle)
        if total_s <= 0:
            total_s = 1800.0
...
async def run_with_execution_timeout(coro: Coroutine[Any, Any, T]) -> T:
    """Run a coroutine with unified timeout and heartbeat."""
