...
HYBRID_SCHEMA_V1 = "xiuxian.vector.hybrid.v1"
VECTOR_SCHEMA_V1 = "xiuxian.vector.search.v1"
TOOL_SEARCH_SCHEMA_V1 = "xiuxian.vector.tool_search.v1"
...
class ToolSearchPayload(BaseModel):
    ...
    def to_router_result(self) -> dict[str, Any]:
        """Build route_result_item dict aligned with xiuxian.router.route_test.v1 (id, name, scores, intents, category; no schema/keywords)."""
...
class ToolRouterResult(BaseModel):
    """Canonical router result passed to CLI and downstream orchestrators (xiuxian.router.route_test.v1 result item)."""
...
- Python entrypoint: retained search helper surfaces in `services.vector.search`
...
def validate_vector_table_contract(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    the list returned by the retained vector result loader (each item = row metadata + id, content).
