...
def _sample_memory() -> tuple[float | None, float | None]:
    return None, None
...
@contextmanager
def _phase_scope(phase: str, extra: dict[str, Any] | None = None):
    del phase
    yield extra if extra is not None else {}
...
async def run_semantic_search(
        ...
        try:
            ...
            if embed_task in done and not embed_task.cancelled():
                ...
                raise EmbeddingUnavailableError(
                    ...
                    "Ensure the embedding HTTP service is running and responsive."
                )
    ...
    if vector is None:
        raise EmbeddingUnavailableError(
            "Embedding returned no vector. Ensure the embedding HTTP service is running."
        )
