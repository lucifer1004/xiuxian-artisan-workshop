"""Skill-tools benchmark snapshot helpers (YAML)."""
...
def default_skills_snapshot_path() -> Path:
    """Return default snapshot path under ``SKILLS_DIR``."""
    return SKILLS_DIR() / "_snapshots" / "benchmark" / "skills_tools.yaml"


def load_skills_snapshot(path: Path) -> dict[str, Any] | None:
...
def save_skills_snapshot(path: Path, payload: dict[str, Any]) -> Path:
