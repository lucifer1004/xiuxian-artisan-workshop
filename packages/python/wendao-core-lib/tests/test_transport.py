...
def _attachment_search_result_table() -> pa.Table:
    return pa.table(
        {
...
def test_transport_client_fetches_attachment_search_info_with_typed_request(monkeypatch) -> None:
    client = WendaoTransportClient(
        WendaoTransportConfig(
            endpoint=WendaoTransportEndpoint(host="127.0.0.1", port=50051),
        )
    ...
    info = client.get_attachment_search_info(
        attachment_search_request(
            "architecture",
            limit=5,
            ext_filters=(" PDF ", "pdf"),
    ...
    assert info == "attachment-search-info"
    assert captured["descriptor"].path == [
        segment.encode("utf-8") for segment in SEARCH_ATTACHMENTS_ROUTE.strip("/").split("/")
    ]
    assert captured["options"].headers == [
        (b"x-wendao-schema-version", b"v1"),
        (WENDAO_SEARCH_QUERY_HEADER.encode("utf-8"), b"architecture"),
        (WENDAO_SEARCH_LIMIT_HEADER.encode("utf-8"), b"5"),
        (WENDAO_ATTACHMENT_SEARCH_CASE_SENSITIVE_HEADER.encode("utf-8"), b"false"),
        (WENDAO_ATTACHMENT_SEARCH_EXT_FILTERS_HEADER.encode("utf-8"), b"pdf"),
        (WENDAO_ATTACHMENT_SEARCH_KIND_FILTERS_HEADER.encode("utf-8"), b"pdf"),
    ]
...
def test_transport_client_reads_attachment_search_rows(monkeypatch) -> None:
    client = WendaoTransportClient(
        WendaoTransportConfig(
            endpoint=WendaoTransportEndpoint(host="127.0.0.1", port=50051),
        )
    )
    expected = _attachment_search_result_table()
    ...
    def fake_read_attachment_search_table(self, request_arg, **kwargs):  # type: ignore[no-untyped-def]
        captured["request"] = request_arg
        return expected

    monkeypatch.setattr(
        WendaoTransportClient, "read_attachment_search_table", fake_read_attachment_search_table
    )
    ...
    assert rows == [
        WendaoAttachmentSearchResultRow(
            name="Architecture PDF",
            path="notes/architecture.md#attachments/design-review.pdf",
            source_id="doc-attachment-1",
            source_stem="architecture",
            source_title="Architecture Notes",
            navigation_target_json='{"kind":"note","path":"notes/architecture.md"}',
            source_path="notes/architecture.md",
            attachment_id="attachment-1",
            attachment_path="assets/design-review.pdf",
            attachment_name="design-review.pdf",
            attachment_ext="pdf",
            kind="pdf",
            score=0.82,
            vision_snippet="System design overview",
        )
    ]
    assert rows[0].attachment_ext == str(expected["attachmentExt"][0].as_py())
    assert rows[0].kind == str(expected["kind"][0].as_py())
    assert captured["request"] == request


def test_parse_attachment_search_rows_accepts_stable_columns() -> None:
    table = _attachment_search_result_table()

    rows = parse_attachment_search_rows(table)

    assert table.column_names == list(ATTACHMENT_SEARCH_COLUMNS)
    assert rows[0].attachment_name == "design-review.pdf"
