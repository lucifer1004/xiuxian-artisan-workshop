...
async def _stdio_loop(session_id: str) -> None:
    ...
    del session_id
    raise RuntimeError(
        "Python stdio loop is decommissioned. Use Rust REPL: `xiuxian-daochang repl --session-id <id>`."
    )
...
def register_gateway_command(parent_app: typer.Typer) -> None:
    ...
    register_requirements("gateway", embedding_index=True)
    ...
    def gateway(
        ...
        """Run Rust gateway (`xiuxian-daochang`) in stdio or webhook mode."""
...
def register_agent_command(parent_app: typer.Typer) -> None:
    ...
    register_requirements("agent", embedding_index=True)
    ...
    def agent(
        ...
        """Interactive chat via Rust `xiuxian-daochang repl`."""
...
def register_channel_command(parent_app: typer.Typer) -> None:
    ...
    register_requirements("channel", embedding_index=True)
    ...
    def channel(
        ...
        """Run Telegram channel via Rust `xiuxian-daochang channel`."""
        ...
        os.environ["XIUXIAN_DAOCHANG_MAX_TOOL_ROUNDS"] = str(int(max_rounds))
