...
mod tests {
    ...
    #[test]
    fn retrieval_rows_to_record_batch_preserves_schema() {
        let batch = retrieval_rows_to_record_batch(&sample_rows()).expect("build batch");
    ...
    #[test]
    fn payload_fetch_record_batch_projects_requested_columns() {
        let batch = retrieval_rows_to_record_batch(&sample_rows()).expect("build batch");
    ...
    #[test]
    fn payload_fetch_record_batch_filters_by_id() {
        let batch = retrieval_rows_to_record_batch(&sample_rows()).expect("build batch");
        let ids = BTreeSet::from(["b".to_string()]);
        let projected = payload_fetch_record_batch(&batch, &[], Some(&ids)).expect("filter batch");
        let rows = retrieval_rows_from_record_batch(&projected).expect("decode rows");
