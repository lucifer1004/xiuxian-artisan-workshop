//! Unit coverage for keyword fusion match utilities.

use aho_corasick::PatternID;
use lance::deps::arrow_array::{Array, StringArray};
use xiuxian_vector::keyword::fusion::{
...
fn automaton_with_phrase_single_word_exact() {
    ...
    let result = count_name_token_matches_and_exact(&ac, "git_commit", exact_id);
    assert!(result.exact_phrase, "commit is substring of git_commit");
    assert_eq!(
        result.token_count, 0,
        "single-word query is only pattern 0, not counted as token"
    );

    let miss = count_name_token_matches_and_exact(&ac, "git_status", exact_id);
    assert_eq!(miss.token_count, 0);
    assert!(!miss.exact_phrase);
}
...
fn automaton_with_phrase_multi_word() {
    ...
    let result = count_name_token_matches_and_exact(&ac, "git_commit", exact_id);
    assert!(result.token_count >= 1, "token match for git and commit");
    assert!(
        !result.exact_phrase,
