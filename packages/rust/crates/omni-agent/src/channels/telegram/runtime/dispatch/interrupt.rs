...
pub(in crate::channels::telegram::runtime) struct ForegroundInterruptController {
    channels: Arc<Mutex<HashMap<String, SessionInterruptState>>>,
}

impl ForegroundInterruptController {
    pub(in crate::channels::telegram::runtime) fn begin_generation(
        &self,
        session_id: &str,
    ) -> watch::Receiver<u64> {
    ...
    pub(in crate::channels::telegram::runtime) fn end_generation(&self, session_id: &str) {
        let mut guard = self.channels.lock().unwrap_or_else(PoisonError::into_inner);
    ...
    pub(in crate::channels::telegram::runtime) fn interrupt(&self, session_id: &str) -> bool {
        let state = {
