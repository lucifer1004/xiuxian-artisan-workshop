...
"#;
    ...
    let entry = KnowledgeScanner::scan_document(&doc_path, temp_dir.path())
        .ok_or_else(|| std::io::Error::other("expected markdown entry"))?;
...
"#,
    ...
    assert!(entries.iter().any(|entry| entry.title == "Document 1"));
    assert!(entries.iter().any(|entry| entry.title == "Document 2"));
    assert!(entries.iter().any(|entry| entry.title == "Document 3"));
    Ok(())
}
...
"#,
    ...
    assert!(
        patterns
            .iter()
            .all(|entry| entry.category == KnowledgeCategory::Pattern)
    );
    Ok(())
}
