...
- [DONE] Wendao configuration ownership is now import-overlay-first: `xiuxian-config-core` owns recursive `imports` plus import-path environment expansion, the checked-in workspace boot config lives at `$PRJ_ROOT/wendao.toml`, `src/settings/` no longer carries a hand-written deep-merge path, `src/bin/wendao/execute/gateway/config.rs` no longer scans TOML text for gateway port/webhook settings, `nix/modules/process.nix` now reads the gateway port through a bounded TOML-aware helper instead of `awk`, and Studio UI persistence now writes `wendao.studio.overlay.toml` instead of rewriting the base `wendao.toml`.
