...
pub(crate) fn plan_repo_entity_build(
    ...
    plan_repo_staged_mutation(
        RepoStagedMutationConfig {
            repo_id,
            table_name_prefix: SearchPlaneService::repo_entity_table_name(repo_id).as_str(),
...
pub(crate) fn repo_entity_file_fingerprints(
    ...
    for row in rows {
        let hasher = row_hash_by_path.entry(row.path().to_string()).or_default();
