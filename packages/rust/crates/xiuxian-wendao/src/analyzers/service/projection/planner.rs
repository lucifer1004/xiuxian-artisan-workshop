...
use crate::analyzers::query::{
    ...
    DocsPlannerWorksetResult, DocsProjectedGapReportQuery, DocsRetrievalHitQuery,
};
...
pub fn build_docs_planner_workset(
    ...
    let queue = build_docs_planner_queue(
    ...
    let ranked_hits = build_docs_planner_rank(
    ...
    for ranked_hit in &ranked_hits {
        items.push(build_docs_planner_item(
...
fn build_docs_planner_workset_balance(
    ...
    let gap_kind_distribution = groups
        .iter()
        .map(|group| DocsPlannerWorksetGapKindBalanceEntry {
            kind: group.kind,
            selected_count: group.selected_count,
        })
        .collect::<Vec<_>>();
    let family_distribution = groups
        .iter()
        .flat_map(|group| group.families.iter())
        .map(|group| DocsPlannerWorksetFamilyBalanceEntry {
            kind: group.kind,
            selected_count: group.selected_count,
        })
        .collect::<Vec<_>>();
    let selection_count = gap_kind_distribution
        .iter()
        .map(|entry| entry.selected_count)
        .sum();
    let gap_kind_spread = spread_for_counts(
        gap_kind_distribution
            .iter()
            .map(|entry| entry.selected_count)
            .collect(),
    );
    let family_spread = spread_for_counts(
        family_distribution
            .iter()
            .map(|entry| entry.selected_count)
            .collect(),
...
fn spread_for_counts(counts: Vec<usize>) -> usize {
