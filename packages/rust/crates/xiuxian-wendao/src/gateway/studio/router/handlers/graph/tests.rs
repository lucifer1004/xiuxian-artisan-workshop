...
fn build_fixture_with_projects(docs: &[(&str, &str)], projects: Vec<UiProjectConfig>) -> Fixture {
...
async fn graph_neighbors_response(
    fixture: &Fixture,
    node_id: &str,
    hops: usize,
    limit: usize,
...
fn assert_graph_neighbors_include_path(response: &GraphNeighborsResponse, suffix: &str) {
    assert!(
...
fn assert_graph_neighbors_include_link_target(response: &GraphNeighborsResponse, suffix: &str) {
    assert!(
        response
            .links
            .iter()
...
fn graph_neighbors_snapshot_payload(response: GraphNeighborsResponse) -> serde_json::Value {
    json!({
        "center": {
