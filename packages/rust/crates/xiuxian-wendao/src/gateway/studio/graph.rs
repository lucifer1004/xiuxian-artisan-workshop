...
pub(crate) async fn node_neighbors(
    ...
    let metadata = resolve_metadata(index.as_ref(), id)
        .ok_or_else(|| StudioApiError::not_found(format!("Node not found: {id}")))?;
    let incoming = index
        .neighbors(id, LinkGraphDirection::Incoming, 1, 128)
        .into_iter()
        .map(|neighbor| neighbor.path)
        .collect();
    let outgoing = index
        .neighbors(id, LinkGraphDirection::Outgoing, 1, 128)
        .into_iter()
        .map(|neighbor| neighbor.path)
        .collect();
    let two_hop = index
        .neighbors(id, LinkGraphDirection::Both, 2, 256)
        .into_iter()
    ...
    Ok(NodeNeighbors {
        node_id: metadata.path.clone(),
        name: display_name(&metadata),
        node_type: classify_node_type(metadata.path.as_str()),
        incoming,
        outgoing,
        two_hop,
    })
...
pub(crate) async fn graph_neighbors(
    ...
    let center = GraphNode {
        ...
        path: center_id.clone(),
        node_type: classify_node_type(center_id.as_str()),
        is_center: true,
        distance: 0,
    };
    let mut seen_nodes = HashSet::from([center_id.clone()]);
    let mut nodes = vec![center.clone()];
    let mut links = Vec::new();

    for neighbor in neighbors {
        if seen_nodes.insert(neighbor.path.clone()) {
            nodes.push(GraphNode {
                id: neighbor.path.clone(),
        ...
        let (source, target) = match neighbor.direction {
            LinkGraphDirection::Incoming => (neighbor.path.clone(), center_id.clone()),
            LinkGraphDirection::Outgoing | LinkGraphDirection::Both => {
                (center_id.clone(), neighbor.path.clone())
            }
        };
...
fn resolve_metadata(
    ...
    index
        .metadata(id)
        .or_else(|| index.resolve_metadata_candidates(id).into_iter().next())
}
