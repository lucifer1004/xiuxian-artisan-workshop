...
pub(super) fn apply(
    ...
    match key {
        "match" | "strategy" | "match_strategy" => {
            state.parsed_match_strategy = Some(LinkGraphMatchStrategy::from_alias(value));
            true
        }
        "query" | "q" => {
        ...
        "sort" => {
            ...
            for item in parse_list_values(value) {
                state.parsed_sort_terms.push(parse_sort_term(&item));
                parsed_any = true;
            }
            if !parsed_any {
                state.parsed_sort_terms.push(parse_sort_term(value));
            }
            true
        }
        "case" | "case_sensitive" => {
            state.parsed_case_sensitive = parse_bool(value);
            true
        }
        "limit" | "top" | "n" | "k" => {
            if let Ok(limit) = value.parse::<usize>()
                && limit > 0
            {
                state.parsed_limit_override = Some(limit);
            }
            true
        }
        _ => false,
