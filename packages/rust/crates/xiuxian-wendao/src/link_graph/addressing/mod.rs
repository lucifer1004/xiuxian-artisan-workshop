...
//!
//! ```ignore
//! use crate::link_graph::addressing::{Address, resolve_node};
//!
    ...
    match address {
        ...
        Address::Block {
            ...
            if let Some(node) = find_by_path(nodes, section_path) {
                ...
                if let Some(block) = find_block_in_node(&node, block_addr) {
                    return Some(ResolvedNode {
                        node,
                        doc_id: doc_id.to_string(),
                        migrated_from: None,
...
fn find_by_path_from_id(nodes: &[PageIndexNode], _id: &str) -> Option<PageIndexNode> {
    // TODO: Could parse ID as path components if it looks like a path
    None
}

/// Fallback: try to find by hash when ID lookup failed.
fn find_by_hash_from_id(nodes: &[PageIndexNode], _id: &str) -> Option<PageIndexNode> {
