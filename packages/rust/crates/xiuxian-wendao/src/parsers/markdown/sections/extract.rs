...
fn push_section(
    ...
    lines: &[String],
    source_context: Option<(&Path, &Path)>,
) {
    ...
    let entities = source_context
        .map(|(source_path, root)| {
            extract_link_targets(&section_text, source_path, root).note_links
        })
        .unwrap_or_default();
...
pub(crate) fn extract_sections(body: &str, source_path: &Path, root: &Path) -> Vec<ParsedSection> {
    extract_sections_inner(body, Some((source_path, root)))
}

pub(in crate::parsers::markdown) fn extract_sections_without_source_context(
    body: &str,
) -> Vec<ParsedSection> {
    extract_sections_inner(body, None)
}
