...
use crate::parsers::markdown::sourcepos::sourcepos_to_byte_range;
...
fn parse_reference_target(raw: &str) -> Option<(Option<String>, Option<String>)> {
    ...
    if trimmed.starts_with('#') {
        return Some((None, Some(trimmed.to_string())));
    }
    ...
    Some((Some(target.to_string()), target_address))
}
