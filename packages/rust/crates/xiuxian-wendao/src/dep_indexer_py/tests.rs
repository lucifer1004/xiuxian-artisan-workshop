...
fn test_dependency_config_load() {
    ...
    let Some(py_dep) = config.manifests.iter().find(|d| d.pkg_type == "python") else {
        panic!("expected python dependency in config");
    };
    assert_eq!(py_dep.registry, Some("pip".to_string()));
}
