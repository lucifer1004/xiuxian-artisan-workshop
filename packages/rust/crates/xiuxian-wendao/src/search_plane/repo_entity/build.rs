...
use super::schema::{
    RepoEntityRow, entity_kind_column, hit_json_column, language_column, path_column,
    projected_columns, repo_entity_batches, repo_entity_schema, rows_from_analysis,
    search_text_column, symbol_kind_column,
};

const REPO_ENTITY_EXTRACTOR_VERSION: u32 = 1;

type RepoEntityBuildAction = RepoStagedMutationAction<Vec<RepoEntityRow>>;
type RepoEntityBuildPlan = RepoStagedMutationPlan<Vec<RepoEntityRow>>;
...
async fn ensure_repo_entity_indexes(
...
fn repo_entity_file_fingerprints(
    rows: &[RepoEntityRow],
    documents: &[RepoCodeDocument],
    ...
    for row in rows {
        row_hash_by_path
            ...
            .update(row.fingerprint_token().as_bytes());
    }
...
mod tests {
    ...
    use super::{
        ...
        versioned_repo_entity_table_name,
    };
    use crate::analyzers::{
    ...
    fn plan_repo_entity_build_reuses_table_for_revision_only_refresh() {
        ...
        let rows = rows_from_analysis("alpha/repo", &analysis)
            .unwrap_or_else(|error| panic!("rows: {error}"));
        let file_fingerprints = super::repo_entity_file_fingerprints(&rows, &documents);
        let table_name =
