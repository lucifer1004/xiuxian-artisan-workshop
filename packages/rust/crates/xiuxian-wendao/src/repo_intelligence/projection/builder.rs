...
use crate::repo_intelligence::records::{DocRecord, ExampleRecord, RelationKind};

use super::contracts::{ProjectionInputBundle, ProjectionPageKind, ProjectionPageSeed};
...
fn doc_projection_kind(doc: &DocRecord, targets: &TargetAnchors) -> ProjectionPageKind {
    if doc
        .format
        .as_deref()
        .is_some_and(|format| format.contains("tutorial"))
    {
        ProjectionPageKind::Tutorial
    } else if !targets.symbol_ids.is_empty()
        || doc.format.as_deref().is_some_and(is_reference_like_format)
    {
...
fn is_reference_like_format(format: &str) -> bool {
    let base = format.strip_suffix("_annotation").unwrap_or(format);
    matches!(
        base,
        "modelica_annotation"
            | "modelica_users_guide_reference"
            | "modelica_users_guide_glossary"
            | "modelica_users_guide_documentation"
            | "modelica_users_guide_modelica_code"
            | "modelica_users_guide_icons"
    )
}
