...
use xiuxian_tokenizer::{chunk_text, count_tokens, count_tokens_with_model, truncate};
...
fn test_count_tokens_with_model() {
    match count_tokens_with_model("Hello, world!", "cl100k_base") {
        Ok(value) => assert!(value > 0),
        Err(error) => panic!("expected tokenizer model to initialize: {error}"),
    }
}
