...
mod tests {
    ...
    use super::{
        DEFAULT_SEARCH_DUCKDB_THREADS, DuckDbDatabasePath,
        resolve_search_duckdb_runtime_with_settings,
    };
    use crate::config::test_support;
    use crate::config::{
        DEFAULT_SEARCH_DUCKDB_MATERIALIZE_THRESHOLD_ROWS,
        DEFAULT_SEARCH_DUCKDB_PREFER_VIRTUAL_ARROW, default_search_duckdb_temp_directory,
    };

    #[test]
    fn duckdb_runtime_resolves_relative_paths_and_overrides()
...
"#,
    ...
    #[test]
    fn duckdb_runtime_falls_back_on_blank_or_invalid_values()
    -> Result<(), Box<dyn std::error::Error>> {
        let temp = tempfile::tempdir()?;
        let root = temp.path();
        let config_path = root.join("wendao.toml");
