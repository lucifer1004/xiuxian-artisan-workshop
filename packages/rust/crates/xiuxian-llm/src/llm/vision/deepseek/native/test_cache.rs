...
use crate::llm::vision::preprocess::PreparedVisionImage;

pub(crate) fn build_cache_key_for_tests(
    model_root: &str,
...
pub fn store_markdown_in_cache_for_tests(cache_key: &str, markdown: &str) {
...
pub fn cache_layer_labels_for_tests() -> (&'static str, &'static str) {
    cache::cache_layer_labels_for_tests()
}

/// Test facade for deepseek native cache operations.
pub struct DeepseekNativeCacheTestFacade;
...
impl DeepseekNativeCacheTestFacade {
    ...
    pub fn valkey_get_with(
        ...
        valkey_get_with_for_tests(valkey_url, key_prefix, ttl_secs, io_timeout_ms, key)
    }
    ...
    pub fn normalize_valkey_timeout_ms(io_timeout_ms: u64) -> u64 {
        normalize_valkey_timeout_ms_for_tests(io_timeout_ms)
    }

    pub fn local_get(key: &str) -> Option<String> {
        local_cache_get_for_tests(key)
    }

    pub fn local_set_with_max_entries(key: &str, markdown: &str, max_entries: usize) {
        local_cache_set_with_max_entries_for_tests(key, markdown, max_entries);
    }

    pub fn local_clear() {
        local_cache_clear_for_tests();
    }
    ...
    pub fn normalize_text_view(text: &str) -> Option<String> {
        normalize_cache_text_view_for_tests(text)
    }

    pub fn normalize_text_owned(text: String) -> Option<String> {
        normalize_cache_text_owned_for_tests(text)
    }

    pub fn store_markdown(cache_key: &str, markdown: &str) {
        store_markdown_in_cache_for_tests(cache_key, markdown);
    }

    pub fn cache_layer_labels() -> (&'static str, &'static str) {
        cache_layer_labels_for_tests()
    }
}
