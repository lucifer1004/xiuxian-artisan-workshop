...
#[test]
fn responses_stream_parser_remaps_tool_alias_and_collects_output() -> anyhow::Result<()> {
...
data: [DONE]"#;
    ...
    let parsed = parse_openai_responses_stream(raw, &alias_to_original)?;
...
data: [DONE]"#;
    ...
    let error = match parse_openai_responses_stream(raw, &HashMap::new()) {
        Ok(parsed) => panic!("empty stream should return a parsing error, got: {parsed:?}"),
        Err(error) => error,
    };
...
data: [DONE]"#;
    ...
    let parsed = parse_openai_responses_stream(raw, &HashMap::new())?;
    assert_eq!(
...
#[test]
fn responses_stream_parser_prefers_message_items_over_output_text_done() -> anyhow::Result<()> {
