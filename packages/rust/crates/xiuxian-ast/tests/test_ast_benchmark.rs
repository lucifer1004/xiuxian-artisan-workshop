...
fn test_python_pattern_matching_performance() {
    ...
    for _ in 0..10 {
        // Find all functions
        let _ = xiuxian_ast::find_python_functions(&content);

        // Find all async functions
        let _ = xiuxian_ast::find_python_async_functions(&content);

        // Find all classes
        let _ = xiuxian_ast::find_python_classes(&content);
    }
...
fn test_tree_sitter_decorated_functions_performance() {
    ...
    for _ in 0..10 {
        let mut parser = xiuxian_ast::TreeSitterPythonParser::new();
...
fn test_docstring_extraction_performance() {
    ...
    for _ in 0..20 {
        ...
        let functions = xiuxian_ast::find_python_functions(&content);
        for func in &functions {
            let _ = xiuxian_ast::extract_docstring_from_match(func);
        }
    }
...
fn test_large_python_file_matching() {
    ...
    let functions = xiuxian_ast::find_python_functions(&content);
    let async_functions = xiuxian_ast::find_python_async_functions(&content);
    let classes = xiuxian_ast::find_python_classes(&content);
