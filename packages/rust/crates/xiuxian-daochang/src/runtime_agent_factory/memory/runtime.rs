...
fn apply_memory_runtime_persistence_settings(
    ...
    if let Some(backend) = trimmed_non_empty(runtime_settings.memory.persistence_backend.as_deref())
    {
        memory.persistence_backend = backend.to_string();
    }
    if let Some(url) = trimmed_non_empty(runtime_settings.memory.persistence_valkey_url.as_deref())
    {
        memory.persistence_valkey_url = Some(url.to_string());
    }
    if let Some(prefix) =
        trimmed_non_empty(runtime_settings.memory.persistence_key_prefix.as_deref())
    {
...
fn apply_memory_runtime_recall_gate_settings(
    ...
    if let Some(max_candidates) = runtime_settings.memory.recall_credit_max_candidates {
    ...
    if let Some(every_turns) = runtime_settings.memory.decay_every_turns {
        memory.decay_every_turns = every_turns;
    }
    if let Some(factor) = runtime_settings.memory.decay_factor {
        memory.decay_factor = factor;
    }
    if let Some(threshold) = runtime_settings.memory.gate_promote_threshold {
        memory.gate_promote_threshold = threshold;
    }
    if let Some(threshold) = runtime_settings.memory.gate_obsolete_threshold {
        memory.gate_obsolete_threshold = threshold;
    }
    if let Some(min_usage) = runtime_settings.memory.gate_promote_min_usage {
        memory.gate_promote_min_usage = min_usage;
    }
    if let Some(min_usage) = runtime_settings.memory.gate_obsolete_min_usage {
        memory.gate_obsolete_min_usage = min_usage;
    }
    if let Some(rate) = runtime_settings.memory.gate_promote_failure_rate_ceiling {
        memory.gate_promote_failure_rate_ceiling = rate;
    }
    if let Some(rate) = runtime_settings.memory.gate_obsolete_failure_rate_floor {
        memory.gate_obsolete_failure_rate_floor = rate;
    }
    if let Some(score) = runtime_settings.memory.gate_promote_min_ttl_score {
        memory.gate_promote_min_ttl_score = score;
    }
    if let Some(score) = runtime_settings.memory.gate_obsolete_max_ttl_score {
...
fn apply_memory_runtime_stream_settings(
    ...
    if let Some(stream_name) = trimmed_non_empty(runtime_settings.memory.stream_name.as_deref()) {
        memory.stream_name = stream_name.to_string();
    }
    if let Some(group) = trimmed_non_empty(runtime_settings.memory.stream_consumer_group.as_deref())
    {
        memory.stream_consumer_group = group.to_string();
    }
    if let Some(prefix) = trimmed_non_empty(
        runtime_settings
            .memory
            .stream_consumer_name_prefix
            .as_deref(),
    ) {
        memory.stream_consumer_name_prefix = prefix.to_string();
    }
    if let Some(batch_size) = runtime_settings.memory.stream_consumer_batch_size {
        memory.stream_consumer_batch_size = batch_size;
    }
    if let Some(block_ms) = runtime_settings.memory.stream_consumer_block_ms {
