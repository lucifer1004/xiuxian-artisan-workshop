...
pub(crate) struct DiscordForegroundRuntime {
    ...
    session_turn_queue: SessionTurnQueue,
...
impl DiscordForegroundRuntime {
    pub(crate) async fn spawn_foreground_turn(&mut self, msg: ChannelMessage) {
        ...
        let session_turn_ticket = self.session_turn_queue.enqueue(&session_id);
        let foreground_gate = Arc::clone(&self.foreground_gate);
        ...
        self.foreground_tasks.spawn(async move {
            ...
            let _session_turn_guard = session_turn_ticket.wait_turn().await;
            let session_wait_ms =
                u64::try_from(session_wait_started.elapsed().as_millis()).unwrap_or(u64::MAX);
            if session_wait_ms >= 50 {
                tracing::warn!(
                    event = "discord.foreground.session_wait",
                    wait_ms = session_wait_ms,
                    session_key = %session_key,
                    channel = %channel_name,
                    recipient = %recipient,
                    "discord foreground turn waited for same-session queue"
                );
            }

            let gate_wait_started = Instant::now();
            let Ok(permit) = foreground_gate.acquire_owned().await else {
                tracing::warn!(
                    session_key = %session_key,
                    channel = %channel_name,
                    recipient = %recipient,
                    "discord foreground gate is closed"
                );
                return;
            };
            let gate_wait_ms =
...
pub(crate) fn build_foreground_runtime(
    ...
    let runtime = DiscordForegroundRuntime {
        ...
        session_turn_queue: SessionTurnQueue::default(),
