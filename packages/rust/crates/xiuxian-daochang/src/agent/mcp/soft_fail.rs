...
impl Agent {
    pub(in crate::agent) fn soft_fail_mcp_tool_error_output(
        name: &str,
        tool_call_id: Option<&str>,
        error: &anyhow::Error,
    ) -> Option<ToolCallOutput> {
        soft_fail_mcp_tool_error_output(name, tool_call_id, error)
    }
}

fn soft_fail_mcp_tool_error_output(
    name: &str,
    tool_call_id: Option<&str>,
    error: &anyhow::Error,
) -> Option<ToolCallOutput> {
    ...
    if name == MEMORY_SAVE_TOOL_NAME {
        let error_kind = if xiuxian_mcp::is_timeout_error_message(&lower) {
            "timeout"
        } else {
            "save_failed"
        };
        ...
        return Some(ToolCallOutput::error(
            xiuxian_mcp::degraded_tool_error_payload(
                name,
                None,
                error_kind,
                None,
