...
pub(crate) fn test_memory_reward_signal_sink(
    memory_store: Arc<EpisodeStore>,
) -> Arc<dyn ZhenfaSignalSink> {
    Arc::new(MemoryRewardSignalSink::new(memory_store, None))
}

pub(crate) fn test_memory_reward_signal_sink_with_valkey_backend(
    memory_store: Arc<EpisodeStore>,
    redis_url: &str,
    state_key: String,
    strict_startup: bool,
) -> Result<Arc<dyn ZhenfaSignalSink>> {
    let backend = MemoryStateBackend::Valkey(Box::new(ValkeyMemoryStateStore::new(
        redis_url,
        state_key,
        strict_startup,
    )?));
    Ok(Arc::new(MemoryRewardSignalSink::new(
        memory_store,
