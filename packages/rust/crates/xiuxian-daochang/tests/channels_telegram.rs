...
#[derive(Clone, Default)]
struct PollingMediaState {
    get_updates_calls: Arc<Mutex<usize>>,
    get_file_requests: Arc<Mutex<Vec<serde_json::Value>>>,
}
...
async fn spawn_mock_telegram_polling_media_api()
...
fn telegram_parse_update_uses_caption_for_photo_messages() {
    ...
    let msg = require_some(ch.parse_update_message(&update), "message");
    assert_eq!(msg.content, "please analyze this image");
}
...
fn telegram_parse_update_generates_placeholder_for_photo_without_caption() {
    ...
    let msg = require_some(ch.parse_update_message(&update), "message");
    assert_eq!(msg.content, "[telegram-photo]");
}
...
async fn telegram_listen_enriches_photo_message_with_image_attachment() -> Result<()> {
    ...
    assert!(matches!(
        ...
        Some(xiuxian_daochang::ChannelAttachment::ImageUrl { url }) if url == expected_url.as_str()
    ));
