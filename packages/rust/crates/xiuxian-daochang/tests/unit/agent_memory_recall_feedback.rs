//! Top-level integration tests for `agent::memory_recall_feedback`.

use xiuxian_daochang::test_support::{
    FeedbackOutcome, MemoryRecallPlan, RECALL_FEEDBACK_SOURCE_ASSISTANT,
...
fn parse_explicit_user_feedback_supports_multiple_forms() {
    assert_eq!(
        ...
        Some(FeedbackOutcome::Success)
    );
    assert_eq!(
        parse_explicit_user_feedback("feedback: failure"),
        Some(FeedbackOutcome::Failure)
    );
    assert_eq!(
...
fn resolve_feedback_outcome_prioritizes_user_feedback() {
    ...
    assert_eq!(outcome, FeedbackOutcome::Failure);
...
fn update_feedback_bias_moves_toward_failure() {
    let updated = update_feedback_bias(0.2, FeedbackOutcome::Failure);
    assert!(updated < 0.2);
}
