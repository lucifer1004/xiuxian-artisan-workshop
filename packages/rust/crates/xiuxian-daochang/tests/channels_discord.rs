...
#[tokio::test]
async fn discord_listen_rejects_empty_bot_token() {
    let channel = DiscordChannel::new(String::new(), vec!["*".to_string()], vec![]);
    let (tx, _rx) = tokio::sync::mpsc::channel(1);
    let error = match channel.listen(tx).await {
        Ok(()) => panic!("listen should reject empty bot token"),
        Err(error) => error,
    };
