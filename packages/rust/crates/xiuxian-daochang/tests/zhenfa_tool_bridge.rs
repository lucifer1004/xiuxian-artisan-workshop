//! End-to-end tests for native zhenfa tool bridge integration in `xiuxian-daochang`.
...
async fn run_turn_dispatches_wendao_search_through_native_zhenfa_bridge() -> anyhow::Result<()> {
    ...
    let (llm_server, last_tool_message) = spawn_mock_llm_server(llm_addr, scenario).await?;
    ...
    assert_eq!(
        ...
        Some("call_1"),
        "wendao bridge tool message should preserve original tool_call_id"
    );
...
async fn run_turn_dispatches_qianhuan_reload_through_native_zhenfa_bridge() -> anyhow::Result<()> {
    ...
    assert_eq!(
        seen_tool_message
            ...
            .and_then(serde_json::Value::as_str),
        Some("call_1"),
        "qianhuan bridge tool message should preserve original tool_call_id"
    );
