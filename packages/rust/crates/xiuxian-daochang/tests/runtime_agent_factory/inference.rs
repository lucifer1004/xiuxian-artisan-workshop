...
#[test]
fn resolve_runtime_inference_url_uses_anthropic_provider_default_when_configured() {
    let mut settings = RuntimeSettings::default();
    settings.inference.provider = Some("anthropic".to_string());

    let resolved = match resolve_runtime_inference_url(&settings, &[]) {
        Ok(resolved) => resolved,
        Err(error) => panic!("anthropic provider default should resolve: {error}"),
    };
    assert_eq!(resolved, "https://api.anthropic.com/v1/messages");
}

#[test]
fn resolve_runtime_inference_url_appends_messages_path_for_anthropic_base_url() {
    let mut settings = RuntimeSettings::default();
    settings.inference.provider = Some("anthropic".to_string());
    settings.inference.base_url = Some("https://aiproxy.xin/api".to_string());

    let resolved = match resolve_runtime_inference_url(&settings, &[]) {
        Ok(resolved) => resolved,
        Err(error) => panic!("anthropic base_url should resolve: {error}"),
    };
    assert_eq!(resolved, "https://aiproxy.xin/api/v1/messages");
}
