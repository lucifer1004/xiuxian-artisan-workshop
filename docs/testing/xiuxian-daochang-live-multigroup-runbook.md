---
type: knowledge
title: "修仙道场 (Xiuxian Daochang) Live Multi-Group Runbook"
category: "testing"
tags:
  - testing
  - xiuxian
...
# 修仙道场 (Xiuxian Daochang) Live Multi-Group Runbook

This runbook standardizes live black-box validation on three real Telegram groups (`Test1`, `Test2`, `Test3`).

## 1. Preconditions

1. `xiuxian-daochang` webhook runtime is already running.
2. Runtime logs are written to `.run/logs/xiuxian-daochang-webhook.log` (or a known path).
...
python3 scripts/channel/capture_telegram_group_profile.py \
  ...
  --log-file .run/logs/xiuxian-daochang-webhook.log \
...
```bash
python3 scripts/channel/test_xiuxian_daochang_session_matrix.py \
...
python3 scripts/channel/test_xiuxian_daochang_command_events.py \
  --suite core \
...
```bash
python3 scripts/channel/test_xiuxian_daochang_complex_scenarios.py \
  ...
  --max-idle-secs 40 \
  --max-parallel 1 \
  --output-json .run/reports/xiuxian-daochang-memory-evolution-live.json \
  --output-markdown .run/reports/xiuxian-daochang-memory-evolution-live.md
```
...
```bash
python3 scripts/channel/reconstruct_xiuxian_daochang_trace.py \
  ...
  --json-out .run/reports/xiuxian-daochang-trace-reconstruction-live.json \
  --markdown-out .run/reports/xiuxian-daochang-trace-reconstruction-live.md
```
...
4. **Validation:** In the `.run/logs/xiuxian-daochang-webhook.log`, search for `agent.next_turn_hint`. You should see that `xiuxian-memory-engine` caught the previous failure, generated a reflection hint, and `xiuxian-qianhuan` successfully injected it into this second turn's prompt, causing the bot to correct its behavior.
...
3. **Validation:** Check the logs for `session.injection.snapshot_created`. Look at the `dropped_blocks` or `truncated_blocks` metrics. You should see that the massive tool output was truncated by `xiuxian-window` to respect the `max_chars` budget, but the core XML tags like `<genesis_rules>` were preserved.
...
3. `.run/reports/xiuxian-daochang-memory-evolution-live.json`
4. `.run/reports/xiuxian-daochang-trace-reconstruction-live.json`
