...
- Removed outdated hardcoded skill path usage in demo command tests and switched to retained project directory APIs.
...
- Replaced runtime hardcoded `assets/skills` references in key modules with retained directory APIs and skill-path helpers.
...
- More consistent path resolution aligned with retained project config APIs (`get_skills_dir()`).
