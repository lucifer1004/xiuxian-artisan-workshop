...
Architecture:
    ...
    - Called via the shared isolated script runner from crawl_url.py
...
Usage:
    # Via run_script_command (automatic uv run):
    from skills._shared.isolation import run_script_command
    result = run_script_command(script_root, "engine.py", {"url": "..."})
...
def _handle_smart_action(result: dict, url: str, chunk_plan: list | None = None) -> None:
    ...
    The chunk_plan is generated by the LLM in the main tool runtime environment.
...
def main():
    ...
    if args.stdin and not sys.stdin.isatty():
        ...
        # Use command line args (for run_script_command)
        url = args.url or ""
